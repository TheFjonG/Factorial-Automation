...
public class ModEventHandler {
	 ...
	 public void onPlayerRespawnEvent(PlayerRespawnEvent event){
		 ...
		 event.player.addChatComponentMessage(new ChatComponentText("You've lost some brain cells caused by the damage. Brain XP: " + (int)playerKnowledge.getKnowledgeXP()));
	 }
	 ...
	 public void onEntityLivingDeath(LivingDeathEvent event) {
		 if(event.entityLiving instanceof EntityZombie && event.source.getDamageType().equals("player")){
			 if(random.nextInt(3) == 0){
				 ...
				 EntityItem drop = new EntityItem(event.entity.worldObj,event.entity.posX,event.entity.posY,event.entity.posZ,new ItemStack(Items.BrainFragment));
	 ...
	 public void onPlayerUseItemEvent(PlayerUseItemEvent.Finish event){
		 if(event.item != null && event.item.getItem() == Items.Brain && !event.entityPlayer.worldObj.isRemote){
			 ...
			 if(playerKnowledge.getKnowledgeRank() <=20){
				...
				//TODO:Convert to lang.
				ChatMessageUtil.sendChatMessageToPlayer(event.entityPlayer, "You're loaded with inventions. You need to find a way to press more inventions in!");
